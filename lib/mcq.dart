...
class _MCQTestPageState extends State<MCQTestPage> {
  Future<void> uploadResult() async {
    for (int i = 0; i < questions.length; i++) {
      if ((questions[i]['correct_answer'] != null
              ? questions[i]['correct_answer'].trim()
              : 'null') ==
          (questions[i]['selectedOption'] != null
              ? questions[i]['selectedOption'].trim()
              : 'null')) {
        Map<String, dynamic> newData = {
          'question': questions[i]['question'],
          'correctAnswer': questions[i]['correct_answer'],
          'selectedAnswer': questions[i]['selectedOption'],
          'status': 'correct'
        };
        selectedAnswers.add(newData);
      } else {
        Map<String, dynamic> newData = {
          'question': questions[i]['question'],
          'correctAnswer': questions[i]['correct_answer'],
    ...
    String apiUrl = 'https://maapitambrapvtitijhansi.com/app/api/uploadResult';
    ...
    Map<String, String> requestBody = {
      ...
      'score': finalScore,
      'scoreSheet': jsonEncode(selectedAnswers)
    };
...
//  https://maapitambrapvtitijhansi.com/app/api/quizDetails/Electrician_yash
  Future<Object> fetchResponse() async {
    final url =
        'https://maapitambrapvtitijhansi.com/app/api/quizDetails/${widget.quizName}';
